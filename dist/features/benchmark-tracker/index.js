...
export class BenchmarkTracker {
    ...
    getSummary(model, taskCategory) {
        ...
        const rows = this.db
            .prepare(`SELECT * FROM benchmarks ${whereClause} ORDER BY timestamp DESC`)
            .all(params);
        if (rows.length === 0)
            return null;
    ...
    detectRegressions(model, taskCategory, threshold = 0.2) {
        const alerts = [];
        const rows = this.db
            .prepare(`SELECT * FROM benchmarks WHERE model = $model AND task_category = $category ORDER BY timestamp DESC LIMIT 50`)
            .all({ $model: model, $category: taskCategory });
        if (rows.length < 10)
            return alerts;
    ...
    getAllSummaries() {
        const modelCategoryPairs = this.db
            .prepare('SELECT DISTINCT model, task_category FROM benchmarks')
            .all();
        const summaries = [];
...
export function createBenchmarkTracker(dbPath) {
