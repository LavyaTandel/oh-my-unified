...
describe('AntiDuplication', () => {
    ...
    it('only executes once for the same query from different agents', async () => {
        let executeCount = 0;
        const key = { type: 'code-search', query: 'auth system' };
        const execute = async () => {
            executeCount++;
    ...
    it('returns cached result within TTL', async () => {
        const key = { type: 'doc-lookup', query: 'SystemObserver API' };
    ...
    it('clears stale cache entries after TTL', async () => {
        ...
        const key = { type: 'arch-analysis', query: 'component tree' };
        ...
        const shortKey = { type: 'pattern-find', query: 'race condition' };
    ...
    it('waits for in-flight search from another agent', async () => {
        let resolve;
        const slowPromise = new Promise(r => { resolve = r; });
        const key = { type: 'doc-lookup', query: 'heavy computation' };
    ...
    it('removes only expired entries on clearStale', async () => {
        ...
        const freshKey = { type: 'code-search', query: 'fresh' };
        const staleKey = { type: 'code-search', query: 'stale' };
    ...
    it('sets cached:true on deduplicated results', async () => {
        const key = { type: 'pattern-find', query: 'singleton pattern' };
