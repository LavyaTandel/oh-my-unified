...
describe('createProactiveFallbackHook', () => {
    it('creates hook with required methods', () => {
        const hook = createProactiveFallbackHook({}, {});
    ...
    it('does nothing when disabled', async () => {
        const hook = createProactiveFallbackHook({}, {}, { enabled: false });
        const output = { temperature: 1, topP: 1, topK: 1, maxOutputTokens: undefined, options: {} };
        await hook['chat.params']({ sessionID: 's1', agent: 'odin', model: { id: 'openai/gpt-4o' } }, output);
    ...
    it('respects cooldown period', async () => {
        ...
        hook.recordError('openai/gpt-4o');
        hook.recordError('openai/gpt-4o');
        const output1 = { temperature: 1, topP: 1, topK: 1, maxOutputTokens: undefined, options: {} };
        await hook['chat.params']({ sessionID: 's1', agent: 'odin', model: { id: 'openai/gpt-4o' } }, output1);
        expect(output1.options.fallbackModel).toBe('anthropic/claude-sonnet');
        // Second call within cooldown should not trigger
        const output2 = { temperature: 1, topP: 1, topK: 1, maxOutputTokens: undefined, options: {} };
        await hook['chat.params']({ sessionID: 's1', agent: 'odin', model: { id: 'openai/gpt-4o' } }, output2);
...
//# sourceMappingURL=proactive-fallback.test.js.map