...
function createMockClient(overrides) {
    const promptAsync = mock(async (args) => {
        if (overrides?.promptAsyncImpl)
            return overrides.promptAsyncImpl(args);
        return {};
    });
    const abort = mock(async () => {
        if (overrides?.abortImpl)
            return overrides.abortImpl();
        return {};
    });
...
describe('isRateLimitError', () => {
    ...
    test('returns true for "quota exceeded" in responseBody', () => {
        expect(isRateLimitError({ data: { responseBody: 'quota exceeded' } })).toBe(true);
    });
...
describe('RuntimeFallbackManager session.error', () => {
    ...
    test('triggers fallback on rate-limit session.error', async () => {
        ...
        const call = mocks.promptAsync.mock.calls[0];
...
describe('RuntimeFallbackManager chain exhaustion', () => {
    test('does not call promptAsync when all chain models have been tried', async () => {
        const { client, mocks } = createMockClient();
        const mgr = new RuntimeFallbackManager(client, { orchestrator: ['openai/model-y'] }, true, 3);
        await mgr.handleEvent({
            type: 'message.updated',
            properties: {
                info: {
...
describe('RuntimeFallbackManager session.deleted', () => {
    ...
    test('ignores session.deleted with no sessionID', async () => {
        ...
        const mgr = new RuntimeFallbackManager(client, makeChains(), true, 3);
        await expect(mgr.handleEvent({ type: 'session.deleted', properties: {} })).resolves.toBeUndefined();
    });
...
describe('RuntimeFallbackManager resolveChain cross-agent isolation', () => {
    test('does not use another agent chain when known agent has no configured chain', async () => {
        const { client, mocks } = createMockClient();
        const mgr = new RuntimeFallbackManager(client, {
    ...
    test('uses cross-agent last-resort only when agent name is unknown', async () => {
        const { client, mocks } = createMockClient();
        const mgr = new RuntimeFallbackManager(client, { orchestrator: ['openai/gpt-4o'] }, true, 3);
        await mgr.handleEvent({
            type: 'session.error',
            properties: {
...
describe('RuntimeFallbackManager maxAttempts', () => {
    test('respects maxAttempts limit', async () => {
        ...
        const { client, mocks } = createMockClient();
        const mgr = new RuntimeFallbackManager(client, { orchestrator: ['openai/model-b', 'openai/model-c'] }, true, 1);
        await mgr.handleEvent({
            type: 'message.updated',
            properties: {
                info: {
    ...
    test('allows fallback when within maxAttempts', async () => {
        ...
        const { client, mocks } = createMockClient();
        const mgr = new RuntimeFallbackManager(client, { orchestrator: ['openai/model-a', 'openai/model-b', 'openai/model-c'] }, true, 2);
        await mgr.handleEvent({
            type: 'message.updated',
            properties: {
                info: {
...
describe('createModelFallbackHook', () => {
    test('returns handleEvent and getFallbackLogs', () => {
        const { client } = createMockClient();
        const hook = createModelFallbackHook({ client, directory: '/tmp' }, {}, { enabled: true, chains: { orchestrator: ['openai/gpt-4o'] }, maxAttempts: 3 });
...
//# sourceMappingURL=model-fallback.test.js.map