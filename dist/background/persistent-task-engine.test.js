...
describe('PersistentTaskEngine', () => {
    ...
    test('1. Engine creates task in DB', async () => {
        const result = await engine.launchTask({ agent: 'test-agent', description: 'Test task', category: 'testing' }, {});
    ...
    test('2. getTaskOutput returns output for completed task', async () => {
        const { taskId, sessionId } = await engine.launchTask({ agent: 'finder', description: 'Find output' }, {});
        ...
        engine = makeEngine();
        const result2 = await engine.launchTask({ agent: 'finder', description: 'persistent find' }, {});
    ...
    test('5. cancelTask updates status to cancelled', async () => {
        const { taskId, sessionId } = await engine.launchTask({ agent: 'worker', description: 'Cancel me' }, {});
    ...
    test('8. Concurrent task limit is respected', async () => {
        ...
        expect(engine.launchTask({ agent: 'a3', description: 'third' }, {})).rejects.toThrow(/max concurrent tasks/i);
    });
    // ============================================================
    ...
    test('10. Task persists across engine restart', async () => {
        ...
        try {
            ...
            const { taskId, sessionId } = await e1.launchTask({ agent: 'persister', description: 'survive restart' }, {});
            e1.shutdown();
    ...
    test('14. Parent session ID is stored', async () => {
        const result = await engine.launchTask({ agent: 'child', description: 'child task', parentSessionId: 'parent_123' }, {});
    ...
    test('15. launchTask generates unique IDs', async () => {
        const ids = new Set();
        for (let i = 0; i < 5; i++) {
            const { taskId } = await engine.launchTask({ agent: 'gen', description: `unique ${i}` }, {});
            expect(ids.has(taskId)).toBe(false);
            ids.add(taskId);
        }
    });
});
//# sourceMappingURL=persistent-task-engine.test.js.map