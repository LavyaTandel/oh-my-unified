...
function withRegistry<T>(fn: (reg: TaskRegistry) => T): T {
...
function cleanTempDb(path: string): void {
  try {
...
const sampleTask = (overrides: Partial<TaskRecord> = {}): Omit<TaskRecord, 'createdAt' | 'updatedAt'> => ({
  id: 'bg_001',
  sessionId: 'ses_001',
  agent: 'test-agent',
  status: 'pending',
