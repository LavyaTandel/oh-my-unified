...
import type { ConfidenceLevel, WorkflowPhase, KnowledgeArea } from './types'
...
describe('WorkflowEngine', () => {
  ...
  it('6. getThresholdFor returns correct values for all phases', () => {
    ...
    const e = engine as unknown as { getThresholdFor(p: WorkflowPhase): number }
  ...
  it('10. phase timing is tracked', () => {
    ...
    const state = e as unknown as { state: { startedAt: number; currentPhaseStartedAt: number } }
...
describe('PrometheusRecon', () => {
  ...
  it('15. deploys sub-agent tasks when codebase knowledge is low', () => {
    const knownAreas: KnowledgeArea[] = [
      {
  ...
  it('18. generates no questions when all knowledge is present', () => {
    const gathered = new Map<string, string[]>([['codebase-structure', ['src/', 'tests/']]])
    const questions = recon.generateQuestions(
      gathered,
      ['project-purpose', 'codebase-structure', 'tech-stack'],
    )
    expect(questions.length).toBe(0)
  })

  it('19. generates clarification question when routes are found', () => {
    const gathered = new Map<string, string[]>([['api-routes', ['/api/users', '/api/auth']]])
