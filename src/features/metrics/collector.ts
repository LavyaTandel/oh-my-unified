...
export class MetricsCollector {
  ...
  private costPerModel: Record<string, { inputPerToken: number; outputPerToken: number }>;

  constructor(dbPath: string = ':memory:', options?: { dailyBudget?: number; costPerModel?: Record<string, { inputPerToken: number; outputPerToken: number }> }) {
  ...
  record(event: MetricEvent): void {
    ...
    this.db.prepare(
      `INSERT INTO metrics (type, session_id, agent, model, feature, value, metadata, timestamp)
       VALUES ($type, $sessionId, $agent, $model, $feature, $value, $metadata, $timestamp)`,
    ).run({
      $type: event.type,
  ...
  query(query: MetricQuery): Array<MetricEvent & { id: number }> {
    ...
    const rows = this.db
      .prepare(
        `SELECT * FROM metrics ${whereClause} ORDER BY timestamp DESC LIMIT $limit`,
      )
  ...
  getSummary(query: MetricQuery = {}): MetricSummary {
    ...
    const byTypeRows = this.db
      .prepare(
        `SELECT type, COUNT(*) as count FROM metrics ${whereClause} GROUP BY type`,
      )
      .all(params) as Array<{ type: string; count: number }>;

    const byModelRows = this.db
      .prepare(
        `SELECT model, COUNT(*) as count FROM metrics ${whereClause} ${conditions.length > 0 ? 'AND' : 'WHERE'} model IS NOT NULL GROUP BY model`,
      )
      .all(params) as Array<{ model: string | null; count: number }>;

    const byAgentRows = this.db
      .prepare(
        `SELECT agent, COUNT(*) as count FROM metrics ${whereClause} ${conditions.length > 0 ? 'AND' : 'WHERE'} agent IS NOT NULL GROUP BY agent`,
      )
      .all(params) as Array<{ agent: string | null; count: number }>;

    const byFeatureRows = this.db
      .prepare(
        `SELECT feature, COUNT(*) as count FROM metrics ${whereClause} ${conditions.length > 0 ? 'AND' : 'WHERE'} feature IS NOT NULL GROUP BY feature`,
      )
      .all(params) as Array<{ feature: string | null; count: number }>;

    const avgValueRow = this.db
      .prepare(
        `SELECT AVG(value) as avg FROM metrics ${whereClause} ${conditions.length > 0 ? 'AND' : 'WHERE'} value IS NOT NULL`,
      )
      .get(params) as { avg: number | null } | undefined;

    const totalValueRow = this.db
      .prepare(
        `SELECT SUM(value) as total FROM metrics ${whereClause} ${conditions.length > 0 ? 'AND' : 'WHERE'} value IS NOT NULL`,
      )
      .get(params) as { total: number | null } | undefined;
  ...
  getCostSummary(since?: number): CostSummary {
    ...
    for (const row of tokenRows) {
      ...
      const meta = row.metadata as { inputTokens?: number; outputTokens?: number; cost?: number } | undefined;
  ...
  getMetricsCount(): number {
    const row = this.db
      .prepare<{ total: number }>('SELECT COUNT(*) as total FROM metrics')
      .get();
    return row?.total ?? 0;
  }
...
const DEFAULT_COST_PER_MODEL: Record<string, { inputPerToken: number; outputPerToken: number }> = {
...
export function createMetricsCollector(dbPath?: string, options?: { dailyBudget?: number }): MetricsCollector {
