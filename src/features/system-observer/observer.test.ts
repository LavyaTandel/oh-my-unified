...
import type { ComponentHealth, HealthCheckFn } from './observer'
...
describe('SystemObserver', () => {
  ...
  it('allows registering custom component health checks', async () => {
    const customCheck: HealthCheckFn = () => ({
      name: 'custom-db',
      status: 'degraded',
      lastCheck: Date.now(),
  ...
  it('fires onStatusChange when a component transitions', async () => {
    const transitions: Array<{ component: string; from: string; to: string }> = []
    const o = new SystemObserver({
      events: {
  ...
  it('overall status is critical when any component is down', async () => {
    ...
    const alwaysDown: HealthCheckFn = () => ({
