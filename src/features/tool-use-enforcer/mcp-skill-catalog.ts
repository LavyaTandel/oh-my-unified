...
function describeMcp(name: string, cfg: Record<string, unknown>): string {
  if (cfg.type === 'remote') {
...
export class McpSkillCatalog {
  ...
  constructor(private options?: McpSkillCatalogOptions) {
    this.initialize()
  }
  ...
  generateTaskSuggestions(description: string): string {
    ...
    const lines: string[] = [`Based on "${description}", consider these tools:\n`]
    for (const entry of matched.slice(0, 5)) {
      const badge =
        entry.category === 'mcp' ? 'MCP' : entry.category === 'gstack-skill' ? 'GSKILL' : 'BUILTIN'
  ...
  readUserConfig(): CatalogEntry[] {
    ...
    try {
      ...
      const config = JSON.parse(raw) as { mcp?: Record<string, Record<string, unknown>> }
  ...
  private skillsPaths(): { opencode: string; claude: string } {
    return {
  ...
  private scanSkillDir(dirPath: string, category: 'gstack-skill' | 'builtin'): CatalogEntry[] {
    try {
      ...
      for (const item of items) {
        ...
        const triggers: string[] = [item.toLowerCase()]
  ...
  private addEntry(entry: Omit<CatalogEntry, 'source'>): void {
    // Only used by defaults — all forced to 'default' source
    this.entries.push({ ...entry, source: 'default' })
  }

  private loadDefaultMcps(): void {
    this.addEntry({
      category: 'mcp',
      name: 'clawdi',
      description:
        'Cross-agent long-term memory: preferences, coding habits, named entities, past bugs, architecture decisions',
    ...
    this.addEntry({
      ...
      description:
        'Code knowledge graph: communities, impact, flows, architecture analysis, dependency traversal',
