...
export class AgentSelector {
  private metadata = new Map<string, Partial<AgentMetadata>>();
  private modelCapabilities = new Map<string, string[]>();
  private agentMCPs = new Map<string, string[]>();
  private agentHealth = new Map<string, { errors: number; successes: number; lastActive: number }>();
  private lastSuggestionTime = new Map<string, number>();
  private suggestionCooldownMs = 30_000;
  ...
  getStats(): { total: number; healthy: number; degraded: number; error: number } {
    const agents = this.getAgentList();
    return {
      total: agents.length,
