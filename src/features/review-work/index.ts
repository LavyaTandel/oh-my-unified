...
const REVIEW_AGENTS = [
  { name: 'Goal Verifier', focus: 'Did we build what was asked?', type: 'oracle' as const },
  { name: 'QA Executor', focus: 'Does it actually work?', type: 'executor' as const },
  { name: 'Code Reviewer', focus: 'Is the code well-written?', type: 'oracle' as const },
  { name: 'Security Auditor', focus: 'Is it secure?', type: 'oracle' as const },
  { name: 'Context Miner', focus: 'Did we miss any context?', type: 'executor' as const },
];

export class ReviewWorkManager {
  private sessions = new Map<string, ReviewWorkState>();

  startReview(sessionId: string, goal: string, constraints: string[], changedFiles: string[]): ReviewWorkState {
    const state: ReviewWorkState = {
      sessionId,
      goal,
      constraints,
      changedFiles,
...
OUTPUT: <verdict>PASS|FAIL</verdict> <sources_searched>...</sources_searched> <discovered_context>...</discovered_context> <missed_requirements>...</missed_requirements> <blocking_issues>BLOCKING only</blocking_issues>`,
  ...
  getReport(sessionId: string): string | null {
    ...
    const rows = state.agents
      .map(
        (a, i) =>
          `| ${i + 1} | ${a.agentName} | ${a.focus} | ${a.verdict} | ${a.confidence} |`,
      )
      .join('\n');
