...
function findSlashCommandPartIndex(
...
export function createAutoSlashCommandHook(
  ...
  const processedCommands = new Map<string, boolean>();
  ...
  return {
    'chat.message': async (
      ...
      if (!textWithoutCodeBlocks.trim().startsWith('/')) return;
      ...
      if (!OUR_COMMANDS.has(parsed.command)) return;
    ...
    'command.execute.before': async (
      ...
      // Only handle OUR commands
      if (!OUR_COMMANDS.has(normalizedCommand)) return;
    ...
    event: async (input: { event: { type: string; properties?: unknown } }): Promise<void> => {
      if (input.event.type === 'session.deleted') {
        ...
        const sessionID = (props?.sessionID as string) ?? (props?.info as Record<string, unknown>)?.id as string;
        if (sessionID) {
