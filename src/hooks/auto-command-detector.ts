 ...
 */
export function createAutoCommandDetectorHook(
  _ctx: PluginInput,
  _config: PluginConfig,
  hookConfig?: AutoCommandDetectorConfig,
) {
  const cfg: Required<AutoCommandDetectorConfig> = {
    enabled: true,
   ...
   */
  function detectAgentMentions(tokens: string[]): string[] {
    return tokens
   ...
   */
  function detectCommandTriggers(
    tokens: string[],
    text: string,
  ): CommandSuggestion[] {
   ...
   * suggestions.
   */
  function detect(input: string): CommandSuggestion[] {
  ...
  async function handleMessageBefore(
    ...
    if (suggestions.length > 0) {
      log(
        `[auto-command-detector] ${suggestions.length} suggestion(s) for input`,
      );
      (output as Record<string, unknown>).commandSuggestions = suggestions;
    }
  }
